...
set compiler="arm-linux-gnueabihf-gcc"

REM host_interface selection - SPI = 1 (Default); I2C = 2;
set host_interface=1

echo [96mUsage: build_raspberry_pi_32bit.cmd {armv6^|armv8-a} [spi^|i2c]
echo Arguments: {armv6^|armv8-a} (mandatory)
echo   armv6    - ARMv6 architecture
echo   armv8-a  - ARMv8-A architecture
echo   e.g. to use armv6 as the architecture: build_raspberry_pi_32bit.cmd armv6 spi
echo   e.g. to use armv8-a as the architecture: build_raspberry_pi_32bit.cmd armv8-a spi
...
echo   e.g. to use spi as the host communication interface: build_raspberry_pi_32bit.cmd armv6 spi (default: spi)
echo   e.g. to use i2c as the host communication interface: build_raspberry_pi_32bit.cmd armv8-a i2c[0m
...
:build_platform
cd %build_path% && ^
cmake -DCMAKE_BUILD_TYPE=%build_type% -DHOST_INTERFACE=%host_interface% -DBUILD_32=ON -DARCH=%arch% -DCMAKE_TOOLCHAIN_FILE=raspberry-gcc-toolchain.cmake -G  "Unix Makefiles" ./../../../ && ^
make && ^
cd %original_path% && ^
exit /b || goto:eof