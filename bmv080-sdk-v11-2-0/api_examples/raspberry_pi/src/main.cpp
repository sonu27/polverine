...
int main()
{
    ...
    combridge_status_t combridge_setup_status = E_COMBRIDGE_OK;
...
/* Combridge only: Open a channel via this serial communication interface */
    ...
    combridge_setup_status = combridge_spi_init(&combridge_handle, channel_index, clock_rate_in_hz, spi_mode);
    
#elif HOST_INTERFACE == HOST_INTERFACE_I2C
    channel_index = 1; // dev/I2C-1
    combridge_setup_status = combridge_i2c_init(&combridge_handle, channel_index, BMV080_I2C_ADDRESS, BMV080_I2C_BAUDRATE);
#else
    #error "Unknown host interface. Supported host communication interfaces are SPI and I2C."
#endif

    if (combridge_setup_status != E_COMBRIDGE_OK)
    {
        printf("Setting up a serial communication failed with combridge status %d\n", (int)combridge_setup_status);

        /*  Combridge only: A setup combridge channel must be released */
        combridge_status_t combridge_release_status = combridge_release(&combridge_handle);
        if (combridge_release_status != E_COMBRIDGE_OK)
        {
            printf("Releasing the serial communication failed with combridge status %d\n", (int)combridge_release_status);
            return combridge_release_status;
        }
        return (int)combridge_setup_status;
    }
    ...
    return (int)bmv080_final_status;
}