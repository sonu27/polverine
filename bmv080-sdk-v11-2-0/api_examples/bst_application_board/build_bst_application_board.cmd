...
echo [96mUsage: build_bst_application_board.cmd {mcu_app31} [spi^|i2c]
echo Arguments: {mcu_app31} (mandatory)
echo   mcu_app31 - build SDK example for BST application board v3.1 MCU (Cortex-M4F)
...
echo   e.g. to use spi as the host communication interface on mcu_app31: build_bst_application_board.cmd mcu_app31 spi (default: spi)
echo   e.g. to use i2c as the host communication interface on mcu_app31: build_bst_application_board.cmd mcu_app31 i2c[0m
...
mingw32-make LOCATION=FLASH TARGET=%target% COMM_INTERFACE=%commInterface%
