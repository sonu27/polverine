...
#define I2C_CLK_FREQ          ((uint32_t)(400e3))
...
#define SPI_CS_PIN          4u     /**< Chip select pin on Teensy v4.0 */
/* for rtx1060 i2c driver is limited to <256 */
#define MAX_BYTES_PER_I2C_READ_REQUEST 128
/* Exported functions --------------------------------------------------------*/
...
int8_t combridge_i2c_read_16bit(bmv080_sercom_handle_t handle, uint16_t header, uint16_t *payload, uint16_t payload_length)
{
    ...
    uint16_t payload_byte_length = payload_length * 2;
    ...
    while( (payload_byte_length - bytes_read) > MAX_BYTES_PER_I2C_READ_REQUEST )
    {
        ...
        bytes_read += i2c_device->instance->requestFrom((uint8_t)BMV080_I2C_ADDRESS, (uint8_t)MAX_BYTES_PER_I2C_READ_REQUEST, false);
    ...
    bytes_read += i2c_device->instance->requestFrom((uint8_t)BMV080_I2C_ADDRESS, (uint8_t)(payload_byte_length - payload_index) , true);
    while(i2c_device->instance->available() && (payload_index < bytes_read )) 
    {
        payload_byte[payload_index++] = i2c_device->instance->read();    
    }
