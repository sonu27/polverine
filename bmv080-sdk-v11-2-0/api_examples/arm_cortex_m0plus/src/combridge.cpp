...
/* Private define ------------------------------------------------------------*/
// SPI pin definitions
#define BMV080_PIN_SPI_MISO  (20u)
#define BMV080_PIN_SPI_MOSI  (19u)
#define BMV080_PIN_SPI_SCK   (18u)
#define BMV080_PIN_SPI_SS    (9u)

/* I²C pin definitions */
#define BMV080_PIN_I2C_SDA   (2u)
#define BMV080_PIN_I2C_SCK   (3u)
/* SPI clock frequency */
#define SPI_CLK_FREQ          ((uint32_t)(1e6))
/* I²C clock frequency */
#define I2C_CLK_FREQ          ((uint32_t)(100e3))
/* Maximum number of bytes that are read in one read request */
/* for Arduino frameworks this is typically limited to 256 */
#define MAX_BYTES_PER_I2C_READ_REQUEST 256

/* Private variables ---------------------------------------------------------*/
static MbedSPI bmv080_spi(BMV080_PIN_SPI_MISO, BMV080_PIN_SPI_MOSI, BMV080_PIN_SPI_SCK);
static MbedI2C bmv080_i2c(BMV080_PIN_I2C_SDA, BMV080_PIN_I2C_SCK);
...
void init_spi(spi_device_t *spi_device)
{
    SPISettings spi_settings(SPI_CLK_FREQ, MSBFIRST, SPI_MODE0);
    spi_device->instance = &bmv080_spi;
    spi_device->settings = spi_settings;
    pinMode(BMV080_PIN_SPI_SS, OUTPUT);
    pinMode(BMV080_PIN_SPI_MOSI, OUTPUT);
    pinMode(BMV080_PIN_SPI_SCK, OUTPUT);
    pinMode(BMV080_PIN_SPI_MISO, INPUT);
    digitalWrite(BMV080_PIN_SPI_SS, HIGH);
    spi_device->instance->begin();
}


void init_i2c(i2c_device_t *i2c_device)
{
    i2c_device->instance = &bmv080_i2c;
...
int8_t combridge_spi_read_16bit(bmv080_sercom_handle_t handle, uint16_t header, uint16_t *payload, uint16_t payload_length)
{
    ...
    digitalWrite(BMV080_PIN_SPI_SS, LOW);
...
int8_t combridge_i2c_read_16bit(bmv080_sercom_handle_t handle, uint16_t header, uint16_t *payload, uint16_t payload_length)
{
    ...
    uint32_t payload_byte_length = payload_length * 2;
    ...
    /* Check if payload needs to be split up into several requests */
    while( (payload_byte_length - bytes_read) > MAX_BYTES_PER_I2C_READ_REQUEST )
    {
        /* Read maximum number of bytes allowed per request, no stop condition is generated */
        bytes_read += i2c_device->instance->requestFrom(BMV080_I2C_ADDRESS, MAX_BYTES_PER_I2C_READ_REQUEST, false);
        while(i2c_device->instance->available() && (payload_index < bytes_read) )
        {
            payload_byte[payload_index++] = i2c_device->instance->read();    
        }
    }
    /* Read remaining bytes and issue stop condition */
    bytes_read += i2c_device->instance->requestFrom(BMV080_I2C_ADDRESS, payload_byte_length - payload_index , true);
    while(i2c_device->instance->available() && (payload_index < bytes_read )) 
    {
        payload_byte[payload_index++] = i2c_device->instance->read();    
    }
     /* check if payload was read successfully */
    if(payload_index != payload_byte_length)
    {
...
int8_t combridge_i2c_write_16bit(bmv080_sercom_handle_t handle, uint16_t header, const uint16_t *payload, uint16_t payload_length)
{
    ...
    i2c_device->instance->beginTransmission(BMV080_I2C_ADDRESS);
    if (i2c_device->instance->write((header_adjusted >> 8) & 0xFF) != 1)
    {
        return E_COMBRIDGE_ERROR_WRITE_HEADER;
    } 
    if (i2c_device->instance->write(header_adjusted & 0xFF) != 1)
    {
        return E_COMBRIDGE_ERROR_WRITE_HEADER;
    } 
    ...
    /* Check if payload needs to be split up into several requests */
    while( (payload_byte_length - bytes_written) > MAX_BYTES_PER_I2C_READ_REQUEST )
    {
        /* Write maximum number of bytes allowed per request, no stop condition is generated */
        bytes_written += i2c_device->instance->write((uint8_t *)&payload_swapped[bytes_written], MAX_BYTES_PER_I2C_READ_REQUEST);
    }
    /* Write remaining bytes */
    bytes_written += i2c_device->instance->write( (uint8_t *)&payload_swapped[bytes_written], (int)(payload_byte_length - bytes_written));
     /* check if payload was read successfully */
    if(bytes_written != payload_byte_length)
    {
       return_value = E_COMBRIDGE_ERROR_READ;
    }
...
{
    rtos::ThisThread::sleep_for(period);
