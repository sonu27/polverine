...
JustGage = function(config) {
  ...
  obj.config = {
    ...
    // color of gauge title
    titleFontFamily: kvLookup('titleFontFamily', config, dataset, "Bosch Sans Regular"),
    ...
    valueFontColor: kvLookup('valueFontColor', config, dataset, "#010101"),
        ...
        // color of label showing current value
        valueFontFamily: kvLookup('valueFontFamily', config, dataset, "Bosch Sans Regular"),
  ...
  if (obj.config.reverse) {
    ...
    // BST magic: Check if textRenderer is used, if so apply it to min label
    obj.txtMinimum = min;
    ...
    obj.txtMaximum = max;
    // BST magic: Check if textRenderer is used, if so apply it to max label
    if (obj.config.textRenderer) {
  ...
  if (obj.config.counter === true) {
    ...
    eve.on("raphael.anim.frame." + (obj.level.id), function() {
      ...
      if (obj.config.textRenderer) {
        obj.txtValue.attr("text", obj.config.textRenderer(currentValue));
      } else if (obj.config.humanFriendly) {
...
JustGage.prototype.activateGauge = function(val, max) {
  //BST magic
  this.active = true;
};

JustGage.prototype.deactivateGauge = function() {
  //BST magic
  var obj = this;
  obj.refresh(0);
  obj.active = false;
...
JustGage.prototype.refresh = function(val, max) {
  ...
  color = getColor(val, (val - obj.config.min) / (obj.config.max - obj.config.min), obj.config.levelColors, obj.config.noGradient, obj.config.customSectors);
    ...
    // BST magic
    // overflow values, limits are applied after text is displayed
    // limits for text display are done in the textRenderer()
...
function getColor(val, pct, col, noGradient, custSec) {
  ...
  //BST magic
  //select color based on the part value and not on equi distance
  //the original implementation tried to scale the coloranges between the different colors

  //if ranges are not given then the color is black
  if (custSec.ranges === undefined){
    return "#000000";
  }
  //lower out of range
  if(val < custSec.ranges[0].lo)
  {
    return custSec.ranges[0].color;
  }
  //higher out of range
  if(val > custSec.ranges[custSec.ranges.length-1].hi)
  {
    return custSec.ranges[custSec.ranges.length-1].color;
  }
